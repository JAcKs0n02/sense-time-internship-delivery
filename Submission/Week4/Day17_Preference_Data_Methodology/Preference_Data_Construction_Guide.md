...
机器可读定义以 [`Preference_Taxonomy.json`](Preference_Taxonomy.json) 为准；本节给出标注解释。
...
Day 17 的十组演示记录位于 [`Preference_Pair_Examples.json`](Preference_Pair_Examples.json)。Day 18 可以增加上游分数、标注者和审核者等审计字段，但不得改变三个训练关键字段的语义。
...
随本目录提交的 `Day17_Validation.json` 是对 taxonomy 和 10 组样例运行结构、覆盖度与安全边界校验后的机器可读结果。
