...
def assemble():
    ...
    text += '2026年9月18日实验结果更新版\n\n'
    text += '本文面向实习导师和后续复现者，整合前七周的环境、数据、训练、评估和部署证据，并汇总第八周自动化、蒸馏和部署实测。前七周已有完整阶段报告，主要归档缺口在主入口与跨周总结；模型质量方面仍保留第二周、第五周未达标和第六周端到端可靠性不足的真实结论。第八周已完成冻结数据准备、正式SFT与DPO训练和合并、三模型完整CEval/CMMLU评测及Gemini评分；确定性诊断发现数学和代码短板。两份匿名人工评分、两轮学生蒸馏及前后对比、监督部署实测和主控评分恢复均已完成。蒸馏CEval小幅下降、速度基本持平；部署与流水线分段可用，不构成模型质量晋级。完整干净CUDA环境的一次端到端验证、最终权重备份清理与Git发布仍待完成。\n\n'
    ...
    text += '\n[12] 本周蒸馏前后比较：docs/week8_distillation_comparison.md；监督部署实测：docs/week8_supervised_deployment.md；主控评分恢复：docs/week8_pipeline_score_integration.md。对应证据分别位于reports/week8的phase3_distillation_comparison、phase4_supervised_deployment与phase4_pipeline_score_integration目录。\n'
    (REPORTS/'technical_report.md').write_text(text)
    prefix = text.split('## 第5章')[0]
    counts = {'chapters_1_to_4_chinese_characters': len(re.findall('[\u4e00-\u9fff]',prefix)), 'total_chinese_characters': len(re.findall('[\u4e00-\u9fff]',text)), 'chapter_count': len(re.findall(r'(?m)^## 第[1-8]章',text)), 'status': 'report_experiments_integrated_publication_pending'}
    ...
    write_json(REPORTS/'week8/report_sources.json', {p: sha256(ROOT/p) for p in sources + ['reports/week8/report_supplement.md', 'reports/week8/phase3_distillation_comparison/checks.json', 'reports/week8/phase3_distillation_student_gpu/checks.json', 'reports/week8/phase4_supervised_deployment/checks.json', 'reports/week8/phase4_pipeline_score_integration/verification.json']})
    figures = {
...
def build_docx(text):
    ...
    doc = Document(); doc.core_properties.author=''; doc.core_properties.last_modified_by=''; section = doc.sections[0]
