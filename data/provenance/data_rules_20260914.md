...
## v1.1实现接入记录（2026-09-14）

方法不变；修正机器清单中两份items结构评估集为200条/200条、prompts结构为15条（v1曾误记0）；显式加入已要求保留的历史问题关系文件及哈希。原v1机器协议和规则文档保存在`reports/week8/phase1/frozen_protocol_v1.json`与`frozen_rules_v1.md`。评估侧提取每条全部user题目，候选侧比较全部角色轮次；评分rubric只核验哈希。
