...
副本沿用已脱敏候选并补齐现行入口及最新评估证据；9月21日已将必要输入迁入data，旧评估代码快照及路径映射用于只读核验历史回执；重复过程材料已移出提交树。9月20日的安装验证是历史实测记录，当前结构验证单独记录于RELEASE_VALIDATION.md。当前仓库入口见主README。检查点删除按用户决定暂不处理。
