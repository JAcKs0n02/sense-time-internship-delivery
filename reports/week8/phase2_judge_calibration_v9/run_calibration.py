...
from pro_adapter import validate_response
...
def main():
 ...
 scores={};error=None;calls=0;usage={'prompt_tokens':0,'completion_tokens':0,'total_tokens':0};usage_complete=True
 try:
  start=judge_balance();save('starting_balance.json',{'cny':str(start)})
  for case in cases:
   current=judge_balance()
   if current<Decimal('1') or start-current>=Decimal('0.50') or calls>=plan['max_calls']:raise ValueError('budget limit')
   ...
   if all(type(u.get(k)) is int and u[k]>=0 for k in usage) and u['total_tokens']==u['prompt_tokens']+u['completion_tokens']:
