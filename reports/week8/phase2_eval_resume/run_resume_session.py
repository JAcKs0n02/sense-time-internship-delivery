...
OUT = ROOT / 'logs/formal-resume-20260917-01'
LOCKS = ROOT / 'reports/week8/phase2_eval_resume'
DEADLINE = datetime.datetime.fromisoformat('2026-09-17T08:30:00+00:00').timestamp()
RECEIPT = json.loads((LOCKS / 'target_release.json').read_text())
assert hashlib.sha256((LOCKS / 'target_release.json').read_bytes()).hexdigest() == sys.argv[1]
assert RECEIPT['status'] == 'RESUME_LOCKS_TARGET_AUDIT_PASS'
EXPECTED = {name: row['runtime_lock_sha256'] for name, row in RECEIPT['models'].items()}
assert list(EXPECTED) == ['original_base', 'final_sft', 'final_dpo']
...
for name, digest in EXPECTED.items():
    assert sha256(LOCKS / (name + '_runtime_lock.json')) == digest, name
...
write_json(OUT / 'launch.json', {'host': socket.gethostname(), 'pid': os.getpid(),
    ...
    'script_sha256': sha256(Path(__file__)), 'mode': 'base_custom20_resume_then_sft_dpo_fresh', 'attempts_per_model': 1})
...
try:
    for name in EXPECTED:
        ...
        lock = json.loads((LOCKS / (name + '_runtime_lock.json')).read_text())
        ...
        if name == 'original_base':
            command = [sys.executable, str(ROOT / 'scripts/week8_resume_eval.py'),
                       '--source', str(ROOT / 'reports/week8/phase2_formal_inference/recovered_failure'),
                       '--model', lock['model_path'], '--judge-model', 'deepseek-flash',
                       '--judge-url', 'https://api.deepseek.com', '--output-dir', str(OUT / name)]
