...
def main():
 ...
 import shutil
 shutil.copytree('/root/autodl-tmp/week8-supervised-deployment-20260918/evidence', OUT/'prior_failed_attempt')
 test=subprocess.run([sys.executable,'-m','unittest','discover','-s',str(ROOT/'tests'),'-p','test_week8_deploy.py'],capture_output=True,text=True)
 (OUT/'linux_port_regression.log').write_text(test.stdout+test.stderr)
